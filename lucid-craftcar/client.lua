...
function firstlogin()
    ESX.TriggerServerCallback('propgressbar', function(data)
   
        arabane = data.craftname
        sayideger = data.craftcar
        ...
        if dist < 3.0 then
            ...
            if sayideger > 0 then
                if not craftActive then
                    DrawText3D(ped_spawns.car_ped.x, ped_spawns.car_ped.y, ped_spawns.car_ped.z+2.0,'E continue to make car')
                    text = "K Reset Car 0"
                else
                    text = "E Reset Car 0"     
                end
            ...
            DrawText3D(ped_spawns.car_ped.x, ped_spawns.car_ped.y, ped_spawns.car_ped.z + 1.0,text)

            if IsControlJustPressed(0, 38) then
                ESX.TriggerServerCallback('propgressbar', function(data)
                    if data.craftcar > 0 then
                        if not craftActive then
...
AddEventHandler('lucid:craft-object:openmenu', function()
    ...
    if craftActive then
       ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'arac_parca', {
           title    = 'Car Materials',
           align    = 'left',
           elements = elements
       }, function(data, menu)
...
AddEventHandler('lucid:craft-car:openmenu', function()
    ...
    ESX.TriggerServerCallback('caritemkontrol', function(data)
        if data.count >= 1 then
            table.insert(elements, {label = "Craft: Asea", value = "asea", item = "blueprint_asea"})
            found = true
        end
    end, "blueprint_asea")
    ...
    ESX.TriggerServerCallback('caritemkontrol', function(data)
        if data.count >= 1 then
            table.insert(elements, {label = "Craft: Oracle", value = "oracle", item = "blueprint_oracle"})
            found = true
        end
    end, "blueprint_oracle")


    ESX.TriggerServerCallback('caritemkontrol', function(data)
        if data.count >= 1 then
            table.insert(elements, {label = "Craft: Sultan", value = "sultan", item = "blueprint_sultan"})
            found = true
        end
    end, "blueprint_sultan")
    ...
    if found then
        ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'arac_parca', {
            ...
            local itemler = data.current.item

            arabane = action
            TriggerServerEvent('aracimikaydet', action)
            TriggerEvent('lucid:setup-carcraft', action)
            TriggerServerEvent('lucid-carcraft:removeItem', itemler)
            TriggerServerEvent('nerdeyimkaydet', 1)
            menu.close()
        end, function(data, menu)
            menu.close()
        end)
        ...
        TriggerEvent('notification', "Üzerinde gerekli eşya yok", 2)
    end
...
AddEventHandler('lucid:craft-car:continueCraftCar', function()
    ...
    local progress = sayideger
    local vehName = arabane
...
AddEventHandler('lucid:craft-car:resetCraftCar', function()
    craftActive = false
    TriggerServerEvent('nerdeyimkaydet', 0)
    TriggerServerEvent('aracimikaydet', "null")
    sayideger = 0
    arabane = ""
   -- TriggerServerEvent('ESX:Server:SetMetaData', "carcraftprogress", {progress = 0, veh = nil})
    if(veh ~= nil) then
        DeleteEntity(veh)
    end
    Citizen.Wait(300)
...
AddEventHandler('lucid:setup-carcraft', function(car)
    ...
    for k,v in pairs(car_spawns) do
        if ESX.Game.IsSpawnPointClear(v, 2.5) then
    ...
    if canSpawn then
        ...
        TriggerServerEvent('ESX:Server:SetMetaData', "carcraftprogress", {progress = 1, veh = vehNameInProgress})
        Citizen.Wait(300)


        playerdata = ESX.GetPlayerData()
    else
        TriggerEvent('notification',"All the spawn locations are occopied", 2)
    end
...
function setUpVehicleProgress(progress)
    ...
    if canSpawn then
        ...
        Notify("All the spawn locations are occopied", "error", 3000)
    end
end
        ...
        if craftActive then
            if progress1Active then
                if DoesEntityExist(veh) and IsEntityAVehicle(veh) then
                    ...
                    if distanceToLeftFrontDoor < 1.2 then
                        DrawText3D(leftFrontDoor.x, leftFrontDoor.y, leftFrontDoor.z, "Attach The Left Front Door [E]")
        
                            if IsControlJustPressed(0, 38) then
                             if carryingProgress1Obj then
                                progressOne()
                             else
                                --Notify("You're not holding a door.", "error")
                            end
                        end
                    end
                end
            elseif progress2Active then
                if DoesEntityExist(veh) and IsEntityAVehicle(veh) then
                    ...
                    if distanceToRightFrontDoor < 1.55 then
                        DrawText3D(rightFrontDoor.x, rightFrontDoor.y, rightFrontDoor.z,"E - Attach The Right Front Door")
                        if IsControlJustPressed(0, 38) then
            ...
            elseif progress3Active then
                if DoesEntityExist(veh) and IsEntityAVehicle(veh) then
                    ...
                    if distanceToLeftBackDoor < 1.55 then
                        DrawText3D(leftBackDoor.x, leftBackDoor.y, leftBackDoor.z,"E - Attach The Back Left Door")
            ...
            elseif progress4Active then
                if DoesEntityExist(veh) and IsEntityAVehicle(veh) then
                    ...
                    if distanceToRightBackDoor < 1.55 then
                        DrawText3D(rightBackDoor.x, rightBackDoor.y, rightBackDoor.z,"E - Attach The Back Right Door")
                        if IsControlJustPressed(0, 38) then
            ...
            elseif progress5Active then
                if DoesEntityExist(veh) and IsEntityAVehicle(veh) then
                    ...
                    if distanceToHood < 2.9 then
                        DrawText3D(hoodPos.x, hoodPos.y, hoodPos.z - 0.3,"E - Attach The Boot")
                            if IsControlJustPressed(0, 38) then
                                if carryingProgress5Obj then
                                    progressFive()
                                else
                                    Notify("You're not holding a boot.", "error")
                            end
                        end
                    end
                end
            elseif progress6Active then
                if DoesEntityExist(veh) and IsEntityAVehicle(veh) then
                    ...
                    if distanceToTrunk < 2.9 then
                        DrawText3D(trunkPos.x, trunkPos.y, trunkPos.z - 0.3,"E - Attach The Trunk")
                        if IsControlJustPressed(0, 38) then
                            if carryingProgress6Obj then
                                progressSix()
                            else
                                Notify("You're not holding a trunk.", "error")
                            end
                        end
                    end
                end  
            elseif progress7Active then   
                if DoesEntityExist(veh) and IsEntityAVehicle(veh) then
                    ...
                    if dist < 4.0 then
                         DrawText3D(vehPos.x, vehPos.y, vehPos.z, "E - Complete the car craft")
                         if IsControlJustPressed(0, 38) then
                            local vehicleProps = {}
                            vehicleProps.plate = GeneratePlate()
                            ESX.Game.SetVehicleProperties(veh, vehicleProps)
                            TriggerServerEvent('lucid:insertVehToData',  ESX.Game.GetVehicleProperties(veh))
                            TriggerServerEvent('nerdeyimkaydet', 0)
                            TriggerServerEvent('aracimikaydet', "null")
                            sayideger = 0
                            arabane = ""
                            Notify("You completed the car craft.", "primary", 3000)
...
function progressOne()
    ...
    ESX.Game.Progressbar("l_attach_door", "Attaching door..", math.random(2000, 5000), false, true, {
        ...
        SetVehicleDoorBroken(veh, 4, true) 
        SetVehicleDoorBroken(veh, 5, true) 
        TriggerServerEvent('ESX:Server:SetMetaData', "carcraftprogress", {progress = 2, veh = vehNameInProgress})
        TriggerServerEvent('nerdeyimkaydet', 2)
        progress1Active = false
        carryingProgress1Obj = false
        progress2Active = true
        Citizen.Wait(1000)
...
function progressTwo()
    ...
    newProp = nil
    ClearPedTasks(PlayerPedId())
    ESX.Game.Progressbar("l_attach_door2", "Attaching door..", math.random(2000, 5000), false, true, {
          disableMovement = true,
          disableCarMovement = true,
          disableMouse = false,
          disableCombat = true,
        ...
        SetVehicleDoorBroken(veh, 4, true) 
        SetVehicleDoorBroken(veh, 5, true) 
        TriggerServerEvent('ESX:Server:SetMetaData', "carcraftprogress", {progress = 3, veh = vehNameInProgress})
        TriggerServerEvent('nerdeyimkaydet', 3)
...
function progressThree()
 DeleteEntity(newProp)
    newProp = nil
    ClearPedTasks(PlayerPedId())
    ESX.Game.Progressbar("l_attach_door3", "Attaching door..", math.random(2000, 5000), false, true, {
          disableMovement = true,
          disableCarMovement = true,
          disableMouse = false,
          disableCombat = true,
        ...
        SetVehicleDoorBroken(veh, 4, true) 
        SetVehicleDoorBroken(veh, 5, true) 
        TriggerServerEvent('ESX:Server:SetMetaData', "carcraftprogress", {progress = 4, veh = vehNameInProgress})
        progress3Active = false
        carryingProgress3Obj = false
        progress4Active = true
        Citizen.Wait(1000)
        TriggerServerEvent('nerdeyimkaydet', 4)
...
function progressFour()
    ...
    newProp = nil
    ClearPedTasks(PlayerPedId())
    ESX.Game.Progressbar("l_attach_door4", "Attaching door..", math.random(2000, 5000), false, true, {
          disableMovement = true,
          disableCarMovement = true,
          disableMouse = false,
          disableCombat = true,
        ...
        ClearPedTasksImmediately(PlayerPedId())
        SetVehicleFixed(veh)
        TriggerServerEvent('ESX:Server:SetMetaData', "carcraftprogress", {progress = 5, veh = vehNameInProgress})
        SetVehicleDoorBroken(veh, 4, true) 
        SetVehicleDoorBroken(veh, 5, true) 
        TriggerServerEvent('nerdeyimkaydet', 5)
...
function progressFive()
    ...
    newProp = nil
    ClearPedTasks(PlayerPedId())
    ESX.Game.Progressbar("l_attach_bonnet", " Attaching bonnet and engine..", math.random(2000, 5000), false, true, {
          disableMovement = true,
          disableCarMovement = true,
          disableMouse = false,
          disableCombat = true,
        ...
        ClearPedTasksImmediately(PlayerPedId())
        SetVehicleFixed(veh)
        TriggerServerEvent('ESX:Server:SetMetaData', "carcraftprogress", {progress = 6, veh = vehNameInProgress})
        TriggerServerEvent('nerdeyimkaydet', 6)
...
function progressSix()
    ...
    newProp = nil
    ClearPedTasks(PlayerPedId())
    ESX.Game.Progressbar("l_kapi_tak", "Attaching Trunk..", math.random(2000, 5000), false, true, {
          disableMovement = true,
          disableCarMovement = true,
          disableMouse = false,
          disableCombat = true,
        ...
        ClearPedTasksImmediately(PlayerPedId())
        SetVehicleFixed(veh)
        TriggerServerEvent('ESX:Server:SetMetaData', "carcraftprogress", {progress = 7, veh = vehNameInProgress})
        TriggerServerEvent('nerdeyimkaydet', 7)
...
Citizen.CreateThread(function()
    while true do
        ...
        if craftActive then
            ...
            if dist < 2.4 then
                DrawText3D(coords.x, coords.y, coords.z + 1.0,"E - Craft Material")
                if IsControlJustPressed(0, 38) then
